...
%
% arguments: (input)
%  x - array of data to be consolidated. If multiple (p) columns, then
%      each row of x is interpreted as a single point in a p-dimensional
%      space.
%
%  y - outputs to be aggregated. If y is not supplied (is left empty)
%      then consolidator is similar to unique(x,'rows'), but with a
%      tolerance on which points are distinct. 
%
...
disp(strvcat('This may not be the most recent version of', ...
   'consolidator. If you find this function useful,', ...
   'then I recommend that you download a current copy', ...
   'from Matlab Central.'))
...
% first shift, scale, and then round. 
if tol>0
  xhat = x - repmat(min(x,[],1),n,1);
  xhat = round(xhat/tol);
else
  xhat = x;
end
